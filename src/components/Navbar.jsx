      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-12">
          ...
          <div className="flex-shrink-0 flex items-center space-x-3">
            {isMainPage ? (
              <button onClick={() => scrollToSection('home')} className="flex items-center space-x-2 focus:outline-none cursor-pointer">
                <img
              ...
              {isMainPage ? (
                navLinks.map((link) => (
                  <button
                    ...
                    className={`relative px-3 py-2 text-sm font-medium transition-colors duration-200 hover:text-tealGlow cursor-pointer ${activeSection === link.href ? 'text-tealGlow font-semibold' : 'text-gray-400'}`}
                  >
                    {link.name}
                    {activeSection === link.href && (
                      <motion.div
                        layoutId="activeNavUnderline"
                        className="absolute bottom-0 left-0 right-0 h-0.5 bg-tealGlow"
                ...
                <>
                  {navLinks.map((link) => (
                    <button
                      key={link.name}
                      onClick={() => scrollToSection(link.href)}
                      className={`block w-full px-3 py-3 text-base font-medium transition-colors hover:text-tealGlow hover:bg-white/5 rounded-md cursor-pointer ${activeSection === link.href ? 'text-tealGlow bg-white/5' : 'text-gray-400'}`}
                    >
                      {link.name}
                    </button>
                  ))}
